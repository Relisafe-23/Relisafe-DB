...
export async function createFailureRatePrediction(req, res, next) {
  try {
    ...
    if (field && data.source ==="Field") {
      frpRate = eval(field * qty * dutyCycle + data.frOffsetOperand + data.failureRateOffset);
    } else if (predicted && data.source ==="Predicted") {
      frpRate = eval(predicted * qty * dutyCycle + data.frOffsetOperand + data.failureRateOffset);
    } else if (allocated && data.source ==="Allocated") {
      frpRate = eval(allocated * qty * dutyCycle + data.frOffsetOperand + data.failureRateOffset);
    } else {
      frpRate = eval(otherFr * qty * dutyCycle + data.frOffsetOperand + data.failureRateOffset);
    }
...
export async function updateFailureRatePrediction(req, res, next) {
  try {
    ...
    if(Math.sign(frpRate) === -1){
      ...
      const createFailureRatePrediction = await FailureRatePrediction.findByIdAndUpdate(data.frpId, editData, {
        new: true,
        runValidators: true,
      });
      res.status(201).json({
